...
assert(html.includes('name="_cc" value="Oliver.Huefner@eraneos.com"'));
assert(html.includes('Dr. Oliver Hüfner'));
assert(html.includes('Partner · Eraneos'));
...
assert.equal((html.match(/class="person-with-portrait"/g) || []).length, 2);
...
assert(!/Underwriting|Schadenexpertise|berufsgruppen\/handwerk|crafts-sector/i.test(html));
